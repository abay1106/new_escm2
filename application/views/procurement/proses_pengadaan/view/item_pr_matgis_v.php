...
include(VIEWPATH."procurement/proses_pengadaan/view/item_pr_v.php"); 
 } else { ?>
<div class="row">
  <div class="col-lg-12">
    <div class="ibox float-e-margins">
      <div class="ibox-title">
        <h5>ITEM PERENCANAAN MATERIAL STRATEGIS</h5>
...
<?php /*
          ...
          <div class="col-sm-6">
            <div class="input-group">
              <span class="input-group-btn">
                ...
                <button type="button" data-id="kode_item" id="item_int_btn" data-url="<?php echo site_url('procurement/picker_matgis') ?>" class="btn btn-primary picker sumberdaya_btn integrated">Pilih Material Strategis</button>
      ...
      <div class="form-group">
        ...
        <div class="col-sm-2">
         <input type="hidden" class="form-control money" name="jumlah_item_inp" id="jumlah_item_inp" value="<?php echo $curval ?>">
         <p class="form-control-static" id="jumlah_item_lbl"></p>
    ...
    if(isset($item) && !empty($item)){
      foreach ($item as $key => $value) {
        ...
        <tr>
          ...
          <td>
            <input type="hidden" value="<?php echo $value['ppi_pr_tujuan'] ?>" name="item_tujuan[<?php echo $idnya ?>]" data-no="<?php echo $idnya ?>" class="tujuan_item">
            <?php echo $value['ppi_pr_tujuan'] ?>
          </td>
        </tr>
...
<div class="form-group">
  ...
  <div class="col-sm-3" id="sisa_pagu_inp_div">
    <p class="form-control-static text-right" id="sisa_pagu">
      <?php echo inttomoney($permintaan['pr_sisa_anggaran']) ?></p>
      <input type="hidden" name="sisa_pagu_inp" id="sisa_pagu_inp" 
      value="<?php echo $permintaan['pr_sisa_anggaran'] ?>">
    </div>
  </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
      ...
      if (tipe != 'MULTIPLE' ||  $('#kode_spk').val() == '') {
        ...
        $.ajax({
          ...
          success:function(data){
			...
			$("#tujuan_item").val(mydata.ppi_pr_tujuan);
        ...
        $.ajax({
          ...
          success:function(data){
            ...
            $("#jumlah_item_inp").val(mydata.ppv_remain);
			$("#tujuan_item").val(mydata.ppi_pr_tujuan);
            $("#jumlah_item_lbl").text(parseFloat(mydata.ppv_remain).toLocaleString(undefined, {minimumFractionDigits: 2,maximumFractionDigits: 8}));
    ...
    $(".action_item").click(function(){ 
      ...
      var jumlah_int = $("#jumlah_item_inp").val();
      var harga_satuan_int = $("#harga_satuan_item_inp").autoNumeric('get');
	  var tujuan = $("#tujuan_item").val();
      ...
      if(harga_satuan_int < 1){
		...
		html += "<td><input type='hidden' class='tujuan_item' data-no='"+no+"' name='item_tujuan["+no+"]' value='"+tujuan+"'/>"+tujuan+"</td>";
        if (tipe == 'MULTIPLE') {
        }
