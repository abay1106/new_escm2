...
<form method="post" action="<?php echo site_url($controller_name."/submit_edit_grup_jasa_smbd");?>"  class="form-horizontal">
        ...
        <div class="ibox float-e-margins">
          ...
          <div class="ibox-content">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-10">
               <textarea class="form-control" required name="name_inp[<?php echo $i ?>]" readonly><?php echo $curval ?></textarea>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <p class="form-control-static">
                  ...
                  value="" <?php echo $curval == "t" ? "checked" : ""?> disabled>
                </p>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <select class="form-control select2" id="level_1" name="level1_inp[<?php echo $i ?>]" data-unspsc = "<?php echo $v['unspsc_code'] ?>" data-no = "<?php echo $i ?>">
                </select>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <select class="form-control select2" name="level2_inp[<?php echo $i ?>]" data-unspsc = "<?php echo $v['unspsc_code'] ?>" id="level_2" data-no = "<?php echo $i ?>">
                </select>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <select class="form-control select2" name="level3_inp[<?php echo $i ?>]" data-unspsc = "<?php echo $v['unspsc_code'] ?>" id="level_3" data-no = "<?php echo $i ?>">
                </select>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <select class="form-control select2" name="level4_inp[<?php echo $i ?>]" data-unspsc = "<?php echo $v['unspsc_code'] ?>" id="level_4" data-no = "<?php echo $i ?>">
                </select>
              </div>
            </div>
...
<?php echo buttonsubmit('commodity/katalog/grup_jasa_sumberdaya',lang('back'),lang('save')) ?>
...
<script type="text/javascript">
  $(function () {
    ...
    $.ajax({
          url:"<?php echo site_url('Commodity/dropdown_grup_jasa_smbd?level=1') ?>",
          type:"get",
          dataType:"json",
          success: function(data) { 
             $("select[name*='level1_inp']").append("<option value=''><?php echo lang('choose') ?></option>");
            $.each(data, function(index, row) {
            $("select[name*='level1_inp']").append("<option value='"+row.srv_group_code+"'>"+row.srv_group_code+" - "+row.srv_group_name+"</option>");
          });
  ...
  function loadLevel2AndBelow(no){
    $.ajax({
        url:"<?php echo site_url('Commodity/dropdown_grup_jasa_smbd?parent='); ?>"+$("select[name='level1_inp["+no+"]']").val(),
        type:"get",
        dataType:"json",
        success: function(data) {
          $("select[name='level2_inp["+no+"]']").append("<option value=''><?php echo lang('choose') ?></option>");
        $.each(data, function(index, row) {
          $("select[name='level2_inp["+no+"]']").append("<option value='"+row.srv_group_code+"'>"+row.srv_group_code+" - "+row.srv_group_name+"</option>");
        });
  ...
  function loadLevel3AndBelow(no){
    $.ajax({
        url:"<?php echo site_url('Commodity/dropdown_grup_jasa_smbd?parent='); ?>"+$("select[name='level2_inp["+no+"]']").val(),
        type:"get",
        dataType:"json",
        success: function(data) {
          $("select[name='level3_inp["+no+"]']").append("<option value=''><?php echo lang('choose') ?></option>");
        $.each(data, function(index, row) {
          $("select[name='level3_inp["+no+"]']").append("<option value='"+row.srv_group_code+"'>"+row.srv_group_code+" - "+row.srv_group_name+"</option>");
        });
  ...
  function loadLevel4AndBelow(no){
    $.ajax({
        url:"<?php echo site_url('Commodity/dropdown_grup_jasa_smbd?parent='); ?>"+$("select[name='level3_inp["+no+"]']").val(),
        type:"get",
        dataType:"json",
        success: function(data) {
          $("select[name='level4_inp["+no+"]']").append("<option value=''><?php echo lang('choose') ?></option>");
        $.each(data, function(index, row) {
          $("select[name='level4_inp["+no+"]']").append("<option value='"+row.srv_group_code+"'>"+row.srv_group_code+" - "+row.srv_group_name+"</option>");
        });
