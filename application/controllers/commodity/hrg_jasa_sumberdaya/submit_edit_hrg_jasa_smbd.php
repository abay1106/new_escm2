...
foreach ($post as $key => $value) {
  ...
  if(is_array($value)){
    ...
    foreach ($value as $key2 => $value2) { 
      ...
      $input[$key2]['total_cost']= moneytoint($post['total_price_inp'][$key2]);
...
if ($this->form_validation->run() == FALSE){
  ...
  foreach ($input as $key => $value) {
    ...
    $checkhist = $this->Commodity_m->getSrvSmbdPrice("", $value['srv_catalog_code'])->row_array();
    if ($checkhist['status'] != "R") {
      $hist = $this->Commodity_m->insertSrvHist($value['srv_catalog_code'],true); //insert history catalog Jasa
    }
