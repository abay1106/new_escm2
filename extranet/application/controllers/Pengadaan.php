			...
			public function view(){
				...
				if($last_state == "eauction"){
					...
					if(!empty($penawaran)){
						...
						/* elseif ($penawaran > $last_offer) {
							$data['message'] = "Penawaran harus lebih rendah dari harga penawaran terakhir";
						}*/
...
*/
					...
					// $data['item'] = $this->db
					// ->select('b.*,c.ppm_id,c.value_min')
					// ->where("ptm_number",$tenderid)
					// ->join("prc_eauction_item c","b.ptm_number=c.ppm_id AND 'b.tit_id'::text='c.tit_id'::text","left")
			...
			//end
				...
				// start header
						...
						if($this->db->affected_rows($result) > 0){
							...
							$inn = array(
								...
								"pqm_currency"=>$lastquo["pqm_currency"],
							...
							$rslt = $this->db->insert("prc_tender_quo_main_hist",$inn);
								...
								// 	$input['pqm_att'] = $f;
								// }else{
								// 	$lastatt = $this->db->order_by("pqm_hist_id", "desc")->where(array("pqm_id"=>$post["pqmid"], "pqm_att_quo !="=> NULL))->get("prc_tender_quo_main_hist")->row_array();
								// 	if ($lastatt == NULL) {
								// 		$lastatt = $this->db->where(array("pqm_id"=>$post["pqmid"]))->get("prc_tender_main")->row_array();
								// 	}
								...
								// 	$input['pqm_att_quo'] = $fq;
								// }else{
								// 	$lastattquo = $this->db->order_by("pqm_hist_id", "desc")->where(array("pqm_id"=>$post["pqmid"], "pqm_att_quo !="=> NULL))->get("prc_tender_quo_main_hist")->row_array();
								// 	if ($lastattquo == NULL) {
								...
								// 	var_dump($input['pqm_att_quo']);
								// 	exit();
								// 	$input['pqm_att_quo'] = sdfs;
								// }
				...
				//end of header
					...
					//start of komersial
							...
							// $harga_nego = $this->db
							// ->where(array("pqm_id"=>$no_penawaran["pqm_id"]))
							// ->get("prc_tender_quo_item")
							// ->row_array();
							...
							if($affected == ($num_item-1)){
								...
								for ($i = 1; $i < $num_item; $i++) {
									if($post["qty_".$i."_input"] > 0 && $post["price_".$i."_input"] > 0){
										...
										//tutup sementara
										// if($post["price_".$i."_input"] != $harga_nego['pqi_price']){ 
										// 	//$result = 0;
										// 	// echo $this->db->last_query();
										// 	echo "xx";
										// 	// echo $post["price"];
