...
foreach ($post as $key => $value) {
  ...
  if(is_array($value)){
    ...
    foreach ($value as $key2 => $value2) { 
      ...
      $this->form_validation->set_rules("valid_start_date_inp[$key2]", "lang:valid_start_date #$key2", 'max_length['.DEFAULT_MAXLENGTH.']');
      $this->form_validation->set_rules("valid_end_date_inp[$key2]", "lang:valid_end_date #$key2", 'max_length['.DEFAULT_MAXLENGTH.']');
      ...
      $input[$key2]['unspsc_group_code']=$catalog['unspsc_group_code'];
      $input[$key2]['unspsc_group_name']=$catalog['unspsc_group_name'];
      ...
      $input[$key2]['status']="";
      $input[$key2]['valid_start_date']=$post['valid_start_date_inp'][$key2];
      $input[$key2]['valid_end_date']=$post['valid_end_date_inp'][$key2];
...
if ($this->form_validation->run() == FALSE){
  ...
  foreach ($input as $key => $value) {
    $com = $value['comment'];
    unset($value['comment']);
    $act = $this->Commodity_m->insertDataMatSmbdPrice($value);
    if($act){
  ...
  if (isset($post['matgis']) AND $post['matgis'] == 'true') {
