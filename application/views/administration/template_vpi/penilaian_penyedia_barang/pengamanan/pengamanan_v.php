<div class="wraker wraker-content animated fadeInRight">
  <div class="row">
    <div class="col-lg-12">
      <div class="ibox float-e-margins">
        <div class="ibox-title">
          <h5>Template Pengamanan</h5>
        ...
        <div class="ibox-content">
          ...
          <button id="activate" disabled type="button" class="btn btn-primary" style="margin-left: 10px">
            Aktifkan
          </button>
          <button id="remove" disabled type="button" class="btn btn-primary">
            Nonaktifkan
          </button>
          </span>                

            <table id="pengamanan" class="table table-bordered table-striped"></table>
...
<script type="text/javascript">
  ...
  $(function () {
    ...
    $table.bootstrapTable({
      ...
      url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/pengamanan/data_pengamanan') ?>",
      cookieIdTable:"adm_pengamanan",
      idField:"ap_id",
        ...
        editable: {
          ...
          validate: function(v) {
              if(!v){ return 'Required field!'}
                  ...
                  $.ajax({
                  url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/pengamanan/update?key=urutan&data=') ?>"+v,
                  type:"get"
                });
...
$table.on('check.bs.table  check-all.bs.table', function () {
  ...
  $.ajax({
    url:"<?php echo site_url('Administration/selection/selection_pengamanan') ?>",
    data:param,
    type:"get"
  });
  ...
  //set session ap_id
  $.ajax({
    url:"<?php echo site_url('administration/set_session/ap_id') ?>"+'/'+selected,
    type:"get"
  });
...
function responseHandler(res) {
  $.each(res.rows, function (i, row) {
    row.state = $.inArray(row.ap_id, selections) !== -1;
  });
  return res;
}
...
<div class="modal fade" id="modal_add_pertanyaan" tabindex="-1" role="dialog" aria-labelledby="modal_add_pertanyaanLabel" aria-hidden="true">
  <div class="modal-dialog  modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form method="post" action="<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/pengamanan/submit_add') ?>" id="ap_form" >
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#ap_form').submit(function(e) {
      ...
      $('#ap_form').ajaxSubmit({
        url: '<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/pengamanan/submit_add') ?>', 
        type: 'post',
   ...
   $remove.click(function(e) {
    if (confirm('Apakah anda yakin?') == true) {
      e.preventDefault(); // avoid to execute the actual submit of the form.
           $.ajax({
                 type: "POST",
                 url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/pengamanan/nonaktif');?>",
                 success: function(data)
                 {
      ...
      $activate.click(function(e) {
        if (confirm('Apakah anda yakin?') == true) {
            e.preventDefault(); // avoid to execute the actual submit of the form.
             $.ajax({
                   type: "POST",
                   url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/pengamanan/aktifkan');?>",
                   success: function(data)
                   {
                       alert(data); // show response from the php script.
                       $table.bootstrapTable('refresh');
