...
$input['ppm_project_id'] = !empty($post['proyek_id']) ?
 $post['proyek_id'] : null;
// end
...
$input['ppm_currency']= (isset($post['mata_uang_inp'])) ? $post['mata_uang_inp'] : null;
...
foreach ($post as $key => $value) {
  ...
  if(is_array($value)){
    ...
    foreach ($value as $key2 => $value2) { 
      ...
      if(isset($post['item_kode'][$key2]) && !empty($post['item_kode'][$key2])){
...
if ($this->form_validation->run() == FALSE || $error){
  ...
  if(empty($next_pos_id)){
    ...
    if($act){
      ...
      $hist = array(
        'ppm_id' => $last_id,
        'pph_plus' => $input['ppm_pagu_anggaran'],
        'pph_remain' => $input['ppm_pagu_anggaran'],
