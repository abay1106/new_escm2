...
<form method="post" action="<?php echo site_url($controller_name."/submit_approval_kat_brg_smbd");?>"  class="form-horizontal">
    ...
    <div class="col-lg-12">
      <div class="ibox float-e-margins">
        ...
        <?php $curval = (isset($v['mat_catalog_code'])) ? $v['mat_catalog_code'] : set_value("code_inp[$i]") ?>
          ...
          <?php $curval = (isset($v['group_code'])) ? $v['group_code'] : set_value("code_inp[$i]") ?>
          <div class="form-group">
              ...
              <select disabled class="form-control select2" style="width:100%;" name="group_inp[<?php echo $i ?>]">
               <!--  <option value=""><?php //echo lang('choose') ?></option>
                <?php //foreach($list_group as $key => $val){ 
                  ...
                  <option value="<?php echo $v['group_code'] ?>" selected><?php echo $v['group_code'] ?> - <?php echo $v['group_name'] ?></option>
                </select>
              </div>
            </div>

            <?php $curval = (isset($v['unspsc_group_code'])) ? $v['unspsc_group_code'] : set_value("code_inp[$i]") ?>
          <div class="form-group">
            <label class="col-sm-2 control-label">UNSPSC Group</label>
            <div class="col-sm-10">
              <select disabled class="form-control select2" style="width:100%;" name="group_inp[<?php echo $i ?>]">
                  <option value="<?php echo $v['unspsc_group_code'] ?>" selected><?php echo $v['unspsc_group_code'] ?> - <?php echo $v['unspsc_group_name'] ?></option>
                </select>
              </div>
            </div>
