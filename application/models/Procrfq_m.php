...
class Procrfq_m extends CI_Model {
	...
	public function getEvalMethodDetails($id) {
	...
	public function getItemRFQ($code = "",$tender = "",$tit_code=""){
		...
		//$this->db->join("com_mat_catalog_smbd", "com_mat_catalog_smbd.mat_catalog_code=prc_tender_item.tit_code","left");
	...
	public function replaceVendorStatusRFQ($input){
		...
		if(!empty($input)){
			...
			/*$statusSendUndangan = array(1, null);*/ //old ada yang tambahin tanpa komen //hel
			$this->db->where(array("ptm_number"=>$input['ptm_number'],"pvs_vendor_code"=>$input['pvs_vendor_code']));
		/*	$this->db->or_where_in("pvs_pq_passed", $statusSendUndangan);*/ //old ada yang tambahin tanpa komen //hel
