...
foreach ($myharga as $key => $value) {
	...
	if(in_array($value['ptv_vendor_code'], $vendor_qualified)){
		...
		$head[$value['vendor_name']]['total'] += ($value['pqi_price']*$value['pqi_quantity']) + (($value['pqi_price']*$value['pqi_quantity']) * (($value['pqi_ppn'])/100)) + (($value['pqi_price']*$value['pqi_quantity']) * (($value['pqi_pph'])/100));
...
*/
	...
	if($kontrak['ctr_is_matgis']){

		$hps = $this->Procrfq_m->getHPSRFQ($ptm_number)->row_array();

		$data['hps'] = (isset($hps['hps_total'])) ? $hps['hps_total'] : 0;
