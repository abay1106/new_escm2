      ...
      <div class="ibox float-e-margins">
        ...
        <div class="ibox-content">
          ...
          <button id="remove" disabled type="button" class="btn btn-primary">
            ...
            <table id="k3l" class="table table-bordered table-striped"></table>
...
<script type="text/javascript">
  ...
  $(function () {
    ...
    $table.bootstrapTable({
      ...
      url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/k3l/data_k3l') ?>",
      cookieIdTable:"adm_k3l",
      idField:"ak_id",
        ...
        editable: {
          ...
          validate: function(v) {
              if(!v){ return 'Required field!'}
                  ...
                  $.ajax({
                  url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/k3l/update?key=urutan&data=') ?>"+v,
                  type:"get"
                });
...
$table.on('check.bs.table  check-all.bs.table', function () {
  ...
  $.ajax({
    url:"<?php echo site_url('Administration/selection/selection_k3l') ?>",
    data:param,
    type:"get"
  });
  ...
  //set session ak_id
  $.ajax({
    url:"<?php echo site_url('administration/set_session/ak_id') ?>"+'/'+selected,
    type:"get"
  });
...
function responseHandler(res) {
  $.each(res.rows, function (i, row) {
    row.state = $.inArray(row.ak_id, selections) !== -1;
  });
  return res;
}
...
<div class="modal fade" id="modal_add_pertanyaan" tabindex="-1" role="dialog" aria-labelledby="modal_add_pertanyaanLabel" aria-hidden="true">
  <div class="modal-dialog  modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form method="post" action="<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/k3l/submit_add') ?>" id="ak_form" >
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#ak_form').submit(function(e) {
       e.preventDefault(); // avoid to execute the actual submit of the form.
      $('#ak_form').ajaxSubmit({
        url: '<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/k3l/submit_add') ?>', 
        type: 'post',
   ...
   $remove.click(function(e) {
    if (confirm('Apakah anda yakin?') == true) {
      e.preventDefault(); // avoid to execute the actual submit of the form.
           $.ajax({
                 type: "POST",
                 url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/k3l/nonaktif');?>",
                 success: function(data)
                 {
      ...
      $activate.click(function(e) {
        if (confirm('Apakah anda yakin?') == true) {
          e.preventDefault(); // avoid to execute the actual submit of the form.
           $.ajax({
                 type: "POST",
                 url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_barang/k3l/aktifkan');?>",
                 success: function(data)
                 {
                     alert(data); // show response from the php script.
                     $table.bootstrapTable('refresh');
