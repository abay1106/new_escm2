...
if ($this->form_validation->run() == FALSE){
  ...
  foreach ($input as $key => $value) {
    ...
    $checkhist = $this->Commodity_m->getMatSmbdPrice("", $value['mat_catalog_code'])->row_array();
    if ($checkhist['status'] != "R") {
      $hist = $this->Commodity_m->insertMatHist($value['mat_catalog_code'],true); //insert history catalog barang
    }

    $act = $this->Commodity_m->updateDataMatSmbdPrice($key,$value);
    if($act){
