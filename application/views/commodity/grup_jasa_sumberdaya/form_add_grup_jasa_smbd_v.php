...
<form method="post" action="<?php echo site_url($controller_name."/submit_add_grup_jasa_smbd");?>"  class="form-horizontal">
    ...
    <div class="col-lg-12">
      <div class="ibox float-e-margins">
        <div class="ibox-title">
          <h5>Form Tambah Grup Jasa Sumberdaya</h5>
  ...
  <script type="text/javascript">
	...
	function loadLevel2AndBelow(){
		$.ajax({
				url:"<?php echo site_url('Commodity/dropdown_grup_jasa_smbd?parent='); ?>"+$("#level_1").val(),
				type:"get",
				dataType:"json",
				success: function(data) {
				$.each(data, function(index, row) {
	...
	function loadLevel3AndBelow(){
		$.ajax({
				url:"<?php echo site_url('Commodity/dropdown_grup_jasa_smbd?parent='); ?>"+$("#level_2").val(),
				type:"get",
				dataType:"json",
				success: function(data) {
	...
	function loadLevel4AndBelow(){
		$.ajax({
				url:"<?php echo site_url('Commodity/dropdown_grup_jasa_smbd?parent='); ?>"+$("#level_3").val(),
				type:"get",
				dataType:"json",
				success: function(data) {
				$("#level_4").append("<option value=''>Pilih</option>");
				$.each(data, function(index, row) {
					$("#level_4").append("<option value='"+row.srv_group_code+"'>"+row.srv_group_code+" - "+row.srv_group_name+"</option>");
				});
