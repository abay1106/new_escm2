<?php 
$view = 'vendor/vpi/monitor_pekerjaan/penilaian_vpi_barang/k3ldan5r_v';
...
$data['pertanyaan_k3l'] = $this->Administration_m->getK3l("","barang")->result_array();
$this->db->where('ar_status', "A");
$this->db->order_by('ar_seq', 'asc');
$data['pertanyaan_5r'] = $this->Administration_m->get5r("","barang")->result_array();
...
if (count($prev_data) > 0) {
	...
	$this->db->where('vvk_id', $prev_data['vvk_id']);
	...
	foreach ($data['pertanyaan_k3l'] as $key => $value) {
		...
		if ($value['ak_id'] != $prev_quest_data[$key]['vvks_pertanyaan_id']) {
			...
			$this->db->order_by('ak_seq', 'asc');
			$prev_pertanyaan = $this->Administration_m->getK3l("",'barang')->result_array();
	...
	foreach ($data['pertanyaan_5r'] as $key => $value) {
		if ($value['ar_id'] != $prev_quest_data[$key]['vvks_pertanyaan_id']) {
			...
			$this->db->order_by('ar_seq', 'asc');
			$prev_pertanyaan = $this->Administration_m->get5r("",'barang')->result_array();
...
$this->template($view,"Monitor Penilaian K3L / 5R",$data);