<?php 
$view = 'vendor/vpi/daftar_pekerjaan/penilaian_vpi_barang/pengamanan_v';
...
$data['pertanyaan'] = $this->Administration_m->getPengamanan("","barang")->result_array();
...
if (count($prev_data) > 0) {
	...
	foreach ($data['pertanyaan'] as $key => $value) {
		...
		if ($value['ap_id'] != $prev_quest_data[$key]['vvps_pertanyaan_id']) {
			...
			$this->db->order_by('ap_seq', 'asc');
			$prev_pertanyaan = $this->Administration_m->getPengamanan("",'barang')->result_array();
...
$this->template($view,"Penilaian Pengamanan",$data);