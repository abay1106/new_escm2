...
if(in_array($last_activity, array(1100,1120))){
  ...
  //$check = $this->db->where("ppm_id",$ptm_number)->get("prc_eauction_header")->row_array();
...
*/
    ...
    //$allvend = array_flip($post['rank_inp']);
    //$comp = array_diff($allvend, $post['winner_inp']);
...
if ($last_activity == 1180) {
  ...
  // $win = array_unique($post['winner_inp']);
 ...
 if (count($check_vol) > 0) {
  ...
  foreach ($getVolItem as $key => $vol_value) {
    ...
    $volumeHist = $this->Procplan_m->insertVolumeHist($dataVolume);
          ...
          //end of insert history pembuatan draft kontrak
...
/*
  ...
  */
  ...
  //print_r($return['nextactivity']); echo" </br>";
  //print_r($return['nextposcode']);
