...
foreach ($item as $key => $value) {
  if (!empty($value['group_code'])) {
        if(!in_array(substr($value['group_code'], 0, 4),$data_item)){
         $data_item[] = substr($value['group_code'], 0, 4);
        }
  }else{
    // $this->db->select('unspsc_code');
    // $this->db->where('group_code', substr($value['group_code'], 0, 3));
    $item_smbd = $this->Procrfq_m->getItemRFQ("",$ptm_number)->result_array();
    foreach ($item_smbd as $key => $value) {
      if(!in_array(substr($value['tit_code'], 0, 3),$data_item)){
         $get_group_smbd = substr($value['tit_code'], 0, 3);
         $this->db->select('unspsc_code');
         $this->db->where('group_code',$get_group_smbd);
         $group_smbd = $this->db->get('com_group_smbd')->row_array();
         $data_item[] = substr($group_smbd['unspsc_code'], 0, 4);
