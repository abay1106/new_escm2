    ...
    <div class="col-lg-12">
      <div class="ibox float-e-margins">
        <div class="ibox-title">
          <h5>Template Aspek Penilaian Mutu Pekerjaan dan Personal</h5>
...
<div class="modal fade" id="modal_add_pertanyaan" tabindex="-1" role="dialog" aria-labelledby="modal_add_pertanyaanLabel" aria-hidden="true">
  <div class="modal-dialog  modal-dialog-centered modal-lg" class="overflow-y: initial !important" role="document">
    <div class="modal-content">
      ...
      echo site_url('administration/template_vpi/aspek_penilaian_mutu_pekerjaan_dan_personal/submit_add') ?>"
      id="app_form" >
...
<script type="text/javascript">
  ...
  $(function () {
    ...
    $table.bootstrapTable({
      ...
      url: "<?php echo site_url('administration/template_vpi/aspek_penilaian_mutu_pekerjaan_dan_personal/data_aspek_penilaian_mutu_pekerjaan_dan_personal') ?>",
      cookieIdTable:"adm_aspek_penilaian_mutu",
      idField:"apm_id",
        ...
        editable: {
          ...
          validate: function(v) {
              if(!v){ return 'Required field!'}
                  ...
                  $.ajax({
                  url: "<?php echo site_url('administration/template_vpi/aspek_penilaian_mutu_pekerjaan_dan_personal/update?key=urutan&data=') ?>"+v,
                  type:"get"
                });
      ...
      },
      {
        ...
        //         else{
        //           $.ajax({
        //           url: "<?php echo site_url('administration/template_vpi/aspek_penilaian_mutu_pekerjaan_dan_personal/update?key=category&data=') ?>"+v,
        //           type:"get"
        //         });
        ...
        //         else{
        //           $.ajax({
        //           url: "<?php echo site_url('administration/template_vpi/aspek_penilaian_mutu_pekerjaan_dan_personal/update?key=note&data=') ?>"+v,
        //           type:"get"
        //         });
...
$table.on('check.bs.table  check-all.bs.table', function () {
  ...
  $.ajax({
    url:"<?php echo site_url('Administration/selection/selection_aspek_penilaian_mutu_pekerjaan_dan_personal') ?>",
    data:param,
    type:"get"
  });
  ...
  $.ajax({
    url:"<?php echo site_url('administration/set_session/apm_id') ?>"+'/'+selected,
    type:"get"
  });
...
function responseHandler(res) {
  $.each(res.rows, function (i, row) {
    row.state = $.inArray(row.apm_id, selections) !== -1;
  });
  return res;
}
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#app_form').submit(function(e) {
      ...
      $('#app_form').ajaxSubmit({
        url: '<?php echo site_url('administration/template_vpi/aspek_penilaian_mutu_pekerjaan_dan_personal/submit_add') ?>', 
        type: 'post',
   ...
   $remove.click(function(e) {
      e.preventDefault(); // avoid to execute the actual submit of the form.
           $.ajax({
                 type: "POST",
                 url: "<?php echo site_url('administration/template_vpi/aspek_penilaian_mutu_pekerjaan_dan_personal/nonaktif');?>",
                 success: function(data)
                 {
   ...
   $activate.click(function(e) {
      e.preventDefault(); // avoid to execute the actual submit of the form.
           $.ajax({
                 type: "POST",
                 url: "<?php echo site_url('administration/template_vpi/aspek_penilaian_mutu_pekerjaan_dan_personal/aktifkan');?>",
                 success: function(data)
                 {
                     alert(data); // show response from the php script.
                     $table.bootstrapTable('refresh');
