...
class Commodity_m extends CI_Model {
	...
	public function getMatCatalogSmbd($id = "",$is_matgis = ""){
		...
		$this->db->select("cmc.mat_catalog_code, COALESCE(cmc.mat_group_code,com_group_smbd.group_code) as group_code,com_group_smbd.group_name, COALESCE(cmc.mat_unspsc_group_code,com_group_smbd.unspsc_code) as unspsc_group_code,cg.group_name as unspsc_group_name, cmc.short_description, cmc.long_description, cmc.status,cmc.uom,COALESCE(
			(
			SELECT cmp.total_cost FROM com_mat_price_smbd cmp WHERE cmc.mat_catalog_code = cmp.mat_catalog_code AND cmp.status = 'A' ORDER BY cmp.updated_datetime DESC LIMIT 1
			),0) as total_price,
			(
			SELECT cmp.del_point_id FROM com_mat_price_smbd cmp WHERE cmc.mat_catalog_code = cmp.mat_catalog_code AND cmp.status = 'A' ORDER BY cmp.updated_datetime DESC LIMIT 1
			)as del_point_id,
			CASE status WHEN 'A' THEN 'Aktif' WHEN 'R' THEN 'Revisi' WHEN 'N' THEN 'Nonaktif' ELSE 'Belum Disetujui' END AS status_name,cmc.is_matgis,cmc.image",false)
		...
		$this->db->join("com_group_smbd com_group_smbd","com_group_smbd.group_code = cmc.mat_group_code");

		$this->db->join("com_group cg","cg.group_code = cmc.mat_unspsc_group_code");

		$this->db->where("cg.group_type","M");

		$this->db->order_by("cmc.updated_datetime","desc");

		return $this->db->get("com_mat_catalog_smbd cmc");
	...
	public function getSrvCatalogSmbd($id = ""){
		...
		$this->db->select("csc.*,com_group_smbd.*,cg.group_name as unspsc_group_name,cg.group_code as unspsc_group_code,'' as uom,COALESCE(
			(
			SELECT csp.total_cost FROM com_srv_price_smbd csp WHERE csc.srv_catalog_code = csp.srv_catalog_code AND csp.status = 'A' ORDER BY csp.updated_datetime DESC LIMIT 1
			),0) as total_price,
			CASE status WHEN 'A' THEN 'Aktif' WHEN 'R' THEN 'Revisi' WHEN 'N' THEN 'Nonaktif' ELSE 'Belum Disetujui' END AS status_name",false);
		...
		$this->db->join("com_group_smbd","com_group_smbd.group_code = csc.srv_group_code");

		$this->db->join("com_group cg","cg.group_code = csc.srv_unspsc_group_code");

		$this->db->where("com_group_smbd.group_type","S");

		$this->db->order_by("csc.updated_datetime","desc");

		return $this->db->get("com_srv_catalog_smbd csc");
