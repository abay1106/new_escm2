...
function <?php echo $mod ?>_act_matgis(value, row, index) {
  ...
  return [
    '<a class="btn btn-primary btn-xs action" href="'+link+'/process_matgis/si/'+value+ext+'">',
    butt,
    '</a>  ',
  ].join('');
}
function stat_format1(value,row,index){
  if(!value){
    value='Pembuatan SI';
  }
...
$(function () {
  ...
  $table_pekerjaan_<?php echo $mod ?>.bootstrapTable({
    ...
    url: "<?php echo site_url('data_matgis/data_task_matgis/si/') ?>",
