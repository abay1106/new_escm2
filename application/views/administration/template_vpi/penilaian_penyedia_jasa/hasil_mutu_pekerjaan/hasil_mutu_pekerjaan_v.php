...
<div class="modal fade" id="modal_add_pertanyaan" tabindex="-1" role="dialog" aria-labelledby="modal_add_pertanyaanLabel" aria-hidden="true">
  <div class="modal-dialog  modal-dialog-centered modal-lg" class="overflow-y: initial !important" role="document">
    <div class="modal-content">
      ...
      echo site_url('administration/template_vpi/penilaian_penyedia_jasa/hasil_mutu_pekerjaan/submit_add') ?>"
      id="app_form" >
...
<script type="text/javascript">
  ...
  $(function () {
    ...
    $table.bootstrapTable({
      ...
      url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_jasa/hasil_mutu_pekerjaan/data_hasil_mutu_pekerjaan') ?>",
        ...
        editable: {
          ...
          validate: function(v) {
              if(!v){ return 'Required field!'}
                  ...
                  $.ajax({
                  url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_jasa/hasil_mutu_pekerjaan/update?key=urutan&data=') ?>"+v,
                  type:"get"
                });
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#app_form').submit(function(e) {
      ...
      $('#app_form').ajaxSubmit({
        url: '<?php echo site_url('administration/template_vpi/penilaian_penyedia_jasa/hasil_mutu_pekerjaan/submit_add') ?>', 
        type: 'post',
   ...
   $remove.click(function(e) {
        ...
        if (confirm('Apakah Anda yakin?') == true) {
          e.preventDefault(); // avoid to execute the actual submit of the form.
           $.ajax({
                 type: "POST",
                 url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_jasa/hasil_mutu_pekerjaan/nonaktif');?>",
                 success: function(data)
                 {
   ...
   $activate.click(function(e) {
    if (confirm('Apakah Anda yakin?') == true) {
      ...
      e.preventDefault(); // avoid to execute the actual submit of the form.
       $.ajax({
             type: "POST",
             url: "<?php echo site_url('administration/template_vpi/penilaian_penyedia_jasa/hasil_mutu_pekerjaan/aktifkan');?>",
             success: function(data)
             {
                 alert(data); // show response from the php script.
                 $table.bootstrapTable('refresh');
