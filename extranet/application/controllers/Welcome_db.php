		...
		*/
			...
			public function index()
			{
				if(!empty($this->session->userdata('userid'))){
					...
					$status_aktivasi = $this->db->select("reg_status_name")
					->where('vendor_id', $this->session->userdata('userid'))
					->get("vw_vnd_header")->row()->reg_status_name;
					...
					if (count($check_uploaded_doc_pq) < 1) {

						redirect('home/profile');

					}else{

						if (count($vnd_doc_pq_date) < 1 OR new DateTime($vnd_doc_pq_date->updated_datetime) < new DateTime($template_doc_pq_date)) {
			...
			public function in(){
				...
				if($captcha == $this->session->userdata("digit")){
					...
					$vendor_id = '45763';
					$reg = '1';
					$login = 'silviaayupricilyakristi@gmail.com';
					$name = 'BUMI GORA TEKNIK';
					
					if(!empty($vendor_id)){
						// if($data["reg_isactivate"] == '1'){
							// if(($data["status"] == '9' && $data["reg_status_id"] == '8') || ($data["status"] == '5' && $data["reg_status_id"] == '6')){
							$check_uploaded_doc_pq = $this->db->where(array("vendor_id"=>$vendor_id))->get("vnd_doc_pq")->row();

							if (count($check_uploaded_doc_pq) == 0 AND $reg != '1') {
									$this->session->set_userdata('message', 'Maaf akun Anda sedang menunggu diverifikasi');
							}else{


								$session = $this->db->query("select last_access + '60 minutes'::interval as last_access, NOW() as nows from vnd_session where login_id = '".$login."'")->row_array();
								if(!empty($session)){
									...
									if($session["last_access"] > $session["nows"]){
										...
										$this->session->set_userdata('userid', $vendor_id);
										$this->session->set_userdata('nama_vendor', $name);
										$this->session->set_userdata('login_id', $login);

										$this->db->query("UPDATE vnd_session SET last_access = date_trunc('second', now()), session_id = '".session_id()."', ip_address = '".$_SERVER['REMOTE_ADDR']."' WHERE login_id = '".$login."'");
									...
									$this->db->query("INSERT INTO vnd_session 
										...
										('".session_id()."', '".$_SERVER['REMOTE_ADDR']."', date_trunc('second', now()), '".$login."')");
			...
			// 			// if($data["reg_isactivate"] == '1'){
			// 				// if(($data["status"] == '9' && $data["reg_status_id"] == '8') || ($data["status"] == '5' && $data["reg_status_id"] == '6')){
			// 				$check_uploaded_doc_pq = $this->db->where(array("vendor_id"=>$data["vendor_id"]))->get("vnd_doc_pq")->row();

			// 				if (count($check_uploaded_doc_pq) == 0 AND $data["reg_isactivate"] != '1') {
			// 						$this->session->set_userdata('message', 'Maaf akun Anda sedang menunggu diverifikasi');
			// 				}else{
